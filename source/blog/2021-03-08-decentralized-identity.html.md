...
типа OP_RETURN, поэтому мы также добавим в транзакцию отправку денег обратно на
этот же адрес. Это позволит нам в дальнейшем создавать новые выходы типа
OP_RETURN с этого же адреса.
...
последней транзакции с выходом типа OP_RETURN, имеющей не меньше определённого
...
OP_RETURN (такое должно быть возможно, но я не пробовал воспроизвести).
